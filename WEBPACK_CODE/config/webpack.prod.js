...
module.exports = {
  ...
  output:{
    // 所有文件的输出目录
    // path.resolve()方法可以返回绝对路
    // __dirname：nodejs的变量，当前模块所在文件夹的完整路径；输出目录为dist文件
    path:path.resolve(__dirname,'../dist'), // path：文件的输出路径（要用绝对路径）
    // 入口文件打包输出文件名
    filename:'static/js/main.js',
    // 自动清空上次打包的内容:将path整个目录内容清空，再进行打包
    clean:true,
  },

        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 此处加购，路由跳转前需要发请求，将产品信息通过请求通知服务器，服务器进行相应的存储 -->
...
export default {
  ...
  computed: {
    ...
    skuImageList() {
      // 当服务器数据没有回来，skuInfo这个对象是空对象，则skuInfo.skuImageList为undefined,所以要至少保证skuInfo.skuImageList为空数组
  ...
  methods: {
    ...
    async addShopCart(){
      // 1. 发请求，将产品加入到数据库
      // 此处是派发一个action，发请求后需要知道加购成功还是失败，再进行后续操作
      // 调用仓库的addOrUpdateShopCart，返回的是一个Promise实例 所以需要等待结果
      // 3. 失败--提示用户
      try {
        await this.$store.dispatch('addOrUpdateShopCart',{skuId:this.$route.params.skuid,skuNum:this.skuNum})
        // 2. 服务器存储成功--路由跳转传递简单参数skuNum
        // 产品复杂数据skuInfo利用会话存储：只能存储字符串（将对象JSON.stringify()编码）

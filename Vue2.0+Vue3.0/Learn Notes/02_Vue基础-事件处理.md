...
3. immediate:true：初始化的时候让handler调用一下；（不管数据有没有变化，立即监听一次）
4. 监视的属性必须存在，才能进行监视；
5. 监视的两种写法：
   1. new Vue时传入watch配置；
...
## 6.1 class样式(常用)
   1. 写法：[:class = "xxx"],xxx可以实字符串、对象、数组；
      1. 字符串写法："classA"样式的类名不确定，需要动态指定；
      2. 数组写法：["classA","classB"]要绑定的样式个数和名字不确定；
      3. 对象写法：{classA:isA,classB:isB}要绑定的样式个数和名字确定，但要动态决定用不用；
## 6.2 style样式
